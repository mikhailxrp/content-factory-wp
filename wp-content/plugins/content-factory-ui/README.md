...
4. **Минимальное количество слов** — минимальный объём статьи (по умолчанию 2000)
5. **Тип статьи (angle)** — угол раскрытия темы (инструкция, кейс, сравнение и т.д.)
6. **Ключевые слова** — список ключевых слов через запятую (опционально)
...
```json
{
  ...
  "min_words": 2000,
  "angle": "инструкция",
  "keywords": ["ипотека", "молодая семья", "кредит", "недвижимость"]
}
```
...
- `min_words` — Минимальное количество слов в статье (обязательно, по умолчанию 2000)
- `angle` — Тип/угол раскрытия темы (обязательно, например: инструкция, кейс, сравнение)
- `keywords` — Массив ключевых слов (опционально)
